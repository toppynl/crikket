    ...
    insert: () => ({
      values: (row: Record<string, unknown>) => ({
        returning: () => {
          ...
          return Promise.resolve([{ ...row, createdAt: new Date(), updatedAt: new Date() }])
        },
      }),
    }),
    update: () => ({
      set: (vals: Record<string, unknown>) => ({
        where: () => ({
          returning: () => {
            ...
            return Promise.resolve([{ ...vals, id: "p1", organizationId: "org-1", slug: "alpha", createdAt: new Date(), updatedAt: new Date() }])
          },
        }),
      }),
    }),
...
describe("listProjects", () => {
  it("returns all projects for the org", async () => {
    state.rows = [{ id: "p1", name: "Alpha", slug: "alpha", organizationId: "org-1", description: null, createdAt: new Date(), updatedAt: new Date() }]
...
describe("createProject", () => {
  it("inserts a project and returns it", async () => {
    ...
    const result = await createProject({ organizationId: "org-1", name: "Beta", slug: "beta" })
  ...
  it("normalizes slug to lowercase with hyphens", async () => {
    const { createProject } = await getSvc()
    await createProject({ organizationId: "org-1", name: "Test", slug: "My App 123" })
    expect(state.insertedRow?.slug).toBe("my-app-123")
  })

  it("throws if slug is empty after normalization", async () => {
    const { createProject } = await getSvc()
    await expect(createProject({ organizationId: "org-1", name: "Bad", slug: "  ---  " }))
      .rejects.toThrow(/slug/i)
  })

  it("throws if name is empty", async () => {
    const { createProject } = await getSvc()
    await expect(createProject({ organizationId: "org-1", name: "   ", slug: "ok" }))
...
describe("getProjectBySlug", () => {
  it("returns null when not found", async () => {
    ...
    const result = await getProjectBySlug({ organizationId: "org-1", slug: "missing" })
  ...
  it("returns the project when found", async () => {
    state.rows = [{ id: "p1", name: "Alpha", slug: "alpha", organizationId: "org-1", description: null, createdAt: new Date(), updatedAt: new Date() }]
    const { getProjectBySlug } = await getSvc()
    const result = await getProjectBySlug({ organizationId: "org-1", slug: "alpha" })
...
describe("updateProject", () => {
  it("updates name", async () => {
    ...
    const result = await updateProject({ id: "p1", organizationId: "org-1", name: "New Name" })
...
describe("deleteProject", () => {
  it("runs without error", async () => {
    ...
    await expect(deleteProject({ id: "p1", organizationId: "org-1" })).resolves.toBeUndefined()
  })
})
